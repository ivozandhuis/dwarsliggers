...
outfile = open('huwelijken_gehuwde_medewerkers.csv', 'w', newline='')
...
with open('huwelijken_gehuwde_medewerkers_out.csv', newline='') as infile:
    reader = csv.DictReader(infile)
    for row in reader:
        url = row['aktes'].strip()
        r = openarchLib.showDoc(url)
        r['my_medewerkersnummer'] = row['my_medewerkersnummer.x']
